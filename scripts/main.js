/* ---------- DATA ---------- */
const grids = {
  featured:[['1JYtrTZKKGAnU-3PPPFY4OZ7CHZf9q71m','Brand Launch & Media'],
  ...
  culture:[['1YN8unVXQvkKYPvWhGG8uyXPXGMC2j1wd','RTC Consumer Study'],
           ['13UjWi84Hr44s4s_m6GYGlaLhbNyXnTtt','Sarees & Identity'],
           ['1abLSlqChU_udbrQpmkInTt-NfGCZxoJ9','Comedy Semiotics'],
  ...
  d2c:[['1dXy4YkxZ1FLUkR_qquguQ6HHbFKhLEvO','BigBasket Growth Plan'],
       ['18nty5hdBJOPdEZKTV-WSkgo_nAC03CL6','Honey Basket Audit'],
       ['1Y8ofpvvvAw3g4qRQaFC-ljB_ow4q5SuN','Brand GTM Strategy'],
  ...
  product:[['1T-7olSfenJUWgnX9FcCWcZYpX4w7zw8x','Community Merch Strategy'],
           ['1aNzygAAPLL8u5FwficL0XBBb6Sh9DWbW','Sony LIV Growth Plan'],
           ['1vphsoKdxhU2NXkT6qvgXisWdpFCSL-v-','SCAMPER Framework']],
  saas:[['1NL_Tkw2kA1KShjYEMqxNzRblsvNNB9gv','Hyundai WhatsApp CX'],
        ['1npR9kDzDE-YtzGFgz702TzoGTJymvKvJ','Chatbot GTM'],
        ['13q2Lb2yJ2KBcys1NM1y5yxAJ5-tgS6C-','Media Integration'],
        ['1JipRpFZqfB3RLD5HejDH-J_uUQFen0L9','Festive CRM Strategy'],
        ['1FhZyewK0JhJ3xxJNxlVjFuLRpNXZnSL','Intl WhatsApp GTM'],
...
/* ---------- BUILD GRIDS ---------- */
const makeCard=(id,t)=>`
<a class="project-card" href="https://drive.google.com/file/d/${id}/view" target="_blank" rel="noopener">
  <div class="card-image" style="background-image:url('https://drive.google.com/thumbnail?id=${id}')"></div>
  <div class="card-content"><h3>${t}</h3></div>
</a>`;
document.querySelectorAll('[data-tab-content]').forEach(g=>{
  const key=g.dataset.tabContent;g.innerHTML=(grids[key]||[]).map(([id,t])=>makeCard(id,t)).join('');
});
...
document.querySelectorAll('.tab').forEach(tab=>{
 tab.onclick=()=>{
   ...
   document.querySelectorAll('[data-tab-content]').forEach(g=>{
     g.style.display=g.dataset.tabContent===sel?'grid':'none';
   });
 };
});
...
addEventListener('scroll',()=>{topnav.style.top=scrollY>400?'0':'-70px'});
...
const blurbs={
 ...
 'Performance Ads':'Managed $350k in Meta & Google spend for 2.8–5 ROAS campaigns.',
